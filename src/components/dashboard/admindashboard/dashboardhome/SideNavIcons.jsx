import "../../Dashboard.css";

const SideNavIcons = () => {
  return (
    <div>
      <div className="NavIcons">
        <div className="BLogo SBLogo">
          <img src="src/assets/images/dlogo.png" alt="boctrust-logo" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/ddashboard.png" alt="dashboard" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dprofile.png" alt="profile" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dmyloan.png" alt="loan" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dtransfer.png" alt="transfer" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dadminacc.png" alt="profile" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dwithdraw.png" alt="withdrawer" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/daccount.png" alt="account" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dremita.png" alt="remita" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dreport.png" alt="report" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dmda.png" alt="mda" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dkyc.png" alt="kyc" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dwebsite.png" alt="setting" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dwithdrawmethod.png" alt="method" />
        </div>
        <div className="IconBox">
          <img src="src/assets/images/dusermanage.png" alt="setting" />
        </div>

        <div className="IconBox">
          <img src="src/assets/images/dreport.png" alt="report" />
        </div>
      </div>
    </div>
  );
};

export default SideNavIcons;
